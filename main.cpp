...
static fs::path user_config_dir()
{
    ...
    return fs::path(std::getenv("HOME") ? std::getenv("HOME") : "") /
           "Library" / "Application Support" / "hw";
#else
    return fs::path(std::getenv("HOME") ? std::getenv("HOME") : "") /
           ".config" / "hw";
#endif
...
static std::vector<std::string> list_languages()
{
    ...
    // embedded
    for (auto& kv : EMBEDDED)
        langs.push_back(kv.first);
...
static bool load_user_template(const std::string& lang,
                               std::string& out)
{
    fs::path base = user_data_dir() / "templates";
    std::vector<fs::path> candidates = {base / (lang + ".tmpl"),
                                        base / (lang + ".txt")};
...
struct Args
{
    ...
    std::string alias_hit;  // resolved via alias flags (-c, --cpp,
                            // --zig, etc.)
};
...
static void print_help(const char* prog, const Config& cfg)
{
    std::cout
        << "Usage: " << prog << " [options]\n\n"
        << "Options:\n"
        << "  -l, --lang <name>   Linguagem (ex.: c, cpp, rust, "
           "python)\n"
    ...
    std::cout
        ...
        << "  Config:    ~/Library/Application "
           "Support/hw/config.ini\n"
        << "  Templates: ~/Library/Application "
           "Support/hw/templates/<lang>.tmpl\n";
#else
        << "  Config:    ~/.config/hw/config.ini\n"
        << "  Templates: ~/.config/hw/templates/<lang>.tmpl\n";
#endif
}
...
int main(int argc, char** argv)
{
    ...
    if (!args.lang.empty())
        lang = args.lang;
    else if (!args.alias_hit.empty())
        lang = args.alias_hit;
    else
        lang = cfg.default_lang.empty() ? "c" : cfg.default_lang;
    ...
    if (!args.out_path.empty())
    {
        ...
        {
            std::cerr << "[hw] ERRO: não consegui escrever em '"
                      << args.out_path << "'\n";
            return 1;
        }
